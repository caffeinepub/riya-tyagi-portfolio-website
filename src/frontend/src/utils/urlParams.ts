...
/**
 * Event system for admin token changes
 * Allows components to react when the admin token is updated
 */

type AdminTokenChangeListener = () => void;
const adminTokenChangeListeners: AdminTokenChangeListener[] = [];

/**
 * Register a listener for admin token changes
 * Returns a cleanup function to unregister the listener
 *
 * @param listener - Function to call when admin token changes
 * @returns Cleanup function to remove the listener
 */
export function onAdminTokenChange(listener: AdminTokenChangeListener): () => void {
    adminTokenChangeListeners.push(listener);
    
    // Return cleanup function
    return () => {
        const index = adminTokenChangeListeners.indexOf(listener);
        if (index > -1) {
            adminTokenChangeListeners.splice(index, 1);
        }
    };
}
...
export function emitAdminTokenChange(): void {
    adminTokenChangeListeners.forEach(listener => {
        try {
            ...
            console.error('Error in admin token change listener:', error);
        }
    });
}
